...
const getConvertedValue = value => (amount, log, precision=2) => parseFloat((amount / Math.pow(1024, log)).toFixed(precision));
...
const getHumanReadableString = readable => (bytes, log=Y(getLogValue)(bytes)) => Y(getConvertedValue)(bytes, log) + Y(getUnitLabel)(log)
// Credit card and IMEI validation
const calc = e => (e*2 < 10) ? e*2 : e*2-9;
const luhnValidation = digits => (card) => card.split("").reduce( (sum, cur, i) => i % 2 == 0 ? calc(cur): parseInt(cur) + sum,0) % 10 == 0;
// Example usages
